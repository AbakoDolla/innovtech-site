    ...
    <section id="rendez-vous" className="container py-14 sm:py-20">
      <div className="motion-card relative overflow-hidden rounded-[2rem] border border-cyan-100 bg-[#F4F8FF] p-6 shadow-[0_18px_42px_rgba(20,68,145,0.08)] sm:p-10">
        ...
        <div className="relative grid gap-10 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
          ...
          <form onSubmit={handleSubmit} className="rounded-[1.6rem] border border-white bg-white/90 p-5 shadow-[0_14px_28px_rgba(20,68,145,0.07)] dark:border-blue-300/20 dark:bg-[#0d1f3a] sm:p-7">
            <div className="grid gap-5 sm:grid-cols-2">
              <label className="grid gap-2 text-sm font-bold text-[#081A3C] dark:text-white sm:col-span-2">{t ? "Votre nom" : "Your name"}<input value={name} onChange={(event) => setName(event.target.value)} placeholder={t ? "Ex. Chantal" : "E.g. Chantal"} className="h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100 dark:border-blue-300/20 dark:bg-[#102747] dark:text-white dark:placeholder:text-slate-400 dark:focus:border-cyan-300 dark:focus:ring-cyan-300/15" /></label>
              <label className="grid gap-2 text-sm font-bold text-[#081A3C] dark:text-white sm:col-span-2">{t ? "Votre besoin" : "Your need"}<select value={service} onChange={(event) => setService(event.target.value)} className="h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100 dark:border-blue-300/20 dark:bg-[#102747] dark:text-white dark:focus:border-cyan-300 dark:focus:ring-cyan-300/15">{services.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label>
              <label className="grid gap-2 text-sm font-bold text-[#081A3C] dark:text-white">{t ? "Date souhaitée" : "Preferred date"}<input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="h-12 rounded-xl border border-slate-200 bg-white px-4 text-sm font-medium text-slate-700 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100 dark:border-blue-300/20 dark:bg-[#102747] dark:text-white dark:focus:border-cyan-300 dark:focus:ring-cyan-300/15" /></label>
              <label className="grid gap-2 text-sm font-bold text-[#081A3C] dark:text-white">{t ? "Heure souhaitée" : "Preferred time"}<span className="relative"><Clock3 className="pointer-events-none absolute left-4 top-3.5 h-4 w-4 text-blue-600 dark:text-cyan-200" /><input type="time" value={time} onChange={(event) => setTime(event.target.value)} className="h-12 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-sm font-medium text-slate-700 outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100 dark:border-blue-300/20 dark:bg-[#102747] dark:text-white dark:focus:border-cyan-300 dark:focus:ring-cyan-300/15" /></span></label>
            </div>
            <label className="mt-5 flex items-start gap-3 rounded-xl border border-blue-100 bg-blue-50/60 p-3 text-xs leading-5 text-slate-600 dark:border-blue-300/20 dark:bg-[#102747] dark:text-slate-200"><input type="checkbox" checked={privacyAccepted} onChange={(event) => setPrivacyAccepted(event.target.checked)} className="mt-0.5 h-4 w-4 shrink-0 accent-blue-700" /><span>{t ? <>J’accepte que les informations saisies soient utilisées pour préparer ma demande de rendez-vous via WhatsApp, selon la <Link href="/confidentialite" className="font-extrabold text-blue-700 underline dark:text-cyan-200">politique de confidentialité</Link>.</> : <>I agree that the information entered may be used to prepare my appointment request through WhatsApp, in line with the <Link href="/confidentialite" className="font-extrabold text-blue-700 underline dark:text-cyan-200">privacy policy</Link>.</>}</span></label>
