...
import { productCatalog } from "@/lib/site";
...
export default function Shop({ lang }: { lang: Lang }) {
  ...
  const gallery = [
    { family: "accessories", image: "/manus-storage/innovtech-gallery-accessories_205b8d9b.png", label: t ? "Les essentiels utiles" : "Useful essentials" },
    { family: "connected", image: "/manus-storage/innovtech-gallery-connected_affc08f6.png", label: t ? "Les objets qui suivent votre rythme" : "Objects that keep up with you" },
    { family: "computing", image: "/manus-storage/innovtech-gallery-computing_126c6ec2.png", label: t ? "Votre espace de création" : "Your creative workspace" },
  ];
      ...
      <section className="container py-10 sm:py-14">
        ...
        <div className="mt-9 grid gap-6 lg:grid-cols-3">
          {gallery.filter((item) => category === "all" || item.family === category).map((item, index) => {
            ...
            return <Link href={`/boutique/${product.id}`} key={item.family} className={`group relative overflow-hidden rounded-[1.8rem] border border-slate-100 bg-white shadow-[0_16px_38px_rgba(20,68,145,0.09)] transition duration-300 hover:-translate-y-1.5 hover:shadow-[0_25px_50px_rgba(20,68,145,0.16)] ${index === 1 ? "lg:translate-y-8" : ""}`}>
              <div className="relative overflow-hidden bg-[#ECF5FF] p-2.5"><img src={item.image} alt={product.name[lang]} className="aspect-[4/3] w-full rounded-[1.35rem] object-cover transition duration-500 group-hover:scale-[1.035]" /><span className="absolute left-6 top-6 grid h-11 w-11 place-items-center rounded-xl bg-white/95 text-blue-700 shadow-sm"><Icon className="h-5 w-5" /></span></div>
              <div className="relative p-6"><div className="circuit-lines absolute inset-0 opacity-35" /><p className="relative text-xs font-extrabold uppercase tracking-[0.14em] text-cyan-600">{item.label}</p><h2 className="relative mt-2 font-display text-2xl font-bold tracking-[-0.035em] text-[#081A3C]">{product.name[lang]}</h2><p className="relative mt-3 text-sm leading-6 text-slate-600">{product.description[lang]}</p><span className="relative mt-5 inline-flex items-center gap-1.5 text-sm font-extrabold text-blue-700">{t ? "Voir la catégorie" : "View category"}<ChevronRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></span></div>
            </Link>;
          })}
        </div>
