...
import { ArrowLeft, CheckCircle2, Clock3, Facebook, Link2, MessageCircle, Minus, Plus, Share2, ShieldCheck, ShoppingBag, Truck } from "lucide-react";
    ...
    <div className="detail-spread relative mt-6 overflow-hidden rounded-[2.25rem] bg-[#F7FAFF] p-4 dark:bg-[#0e2340] sm:mt-7 sm:p-8">
      ...
      <div className="relative grid gap-8 lg:grid-cols-[0.95fr_1.05fr] lg:items-center lg:gap-10">
        ...
        <section>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl border border-blue-100 bg-white/80 p-3 text-sm dark:border-blue-300/20 dark:bg-[#0b1b33]"><Clock3 className="h-4 w-4 text-blue-700 dark:text-blue-300" /><p className="mt-2 text-xs font-extrabold uppercase tracking-[.1em] text-slate-500 dark:text-slate-300">{t ? "Réponse" : "Response"}</p><p className="mt-1 text-sm font-bold leading-5 text-[#081A3C] dark:text-white">{t ? commercial.responseTimeFr : commercial.responseTimeEn}</p></div><div className="rounded-2xl border border-blue-100 bg-white/80 p-3 text-sm dark:border-blue-300/20 dark:bg-[#0b1b33]"><Truck className="h-4 w-4 text-blue-700 dark:text-blue-300" /><p className="mt-2 text-xs font-extrabold uppercase tracking-[.1em] text-slate-500 dark:text-slate-300">{t ? "Livraison" : "Delivery"}</p><p className="mt-1 text-sm font-bold leading-5 text-[#081A3C] dark:text-white">{t ? commercial.deliveryInfoFr : commercial.deliveryInfoEn}</p></div></div>
