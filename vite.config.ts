  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      server.middlewares.use("/media", (req, res, next) => {
        ...
        if (!fileName || !filePath.startsWith(mediaDir + path.sep) || !fs.existsSync(filePath)) {
          next();
          return;
        }
      ...
      server.middlewares.use("/manus-storage", (req, res, next) => {
        const fileName = decodeURIComponent(req.url?.replace(/^\//, "") || "").replace(/_[a-f0-9]{8,}(?=\.)/, "");
        const filePath = path.resolve(mediaDir, fileName);
        if (!fileName || !filePath.startsWith(mediaDir + path.sep) || !fs.existsSync(filePath)) {
          next();
          return;
...
const plugins = [react(), tailwindcss(), jsxLocPlugin(), vitePluginManusRuntime(), vitePluginManusDebugCollector(), vitePluginLocalMedia(), vitePluginStorageProxy()];
    ...
    rollupOptions: {
      output: {
        manualChunks(id) {
          ...
          if (id.includes("@radix-ui") || id.includes("lucide-react") || id.includes("embla")) return "vendor-ui";
